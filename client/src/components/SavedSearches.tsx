...
export default function SavedSearches({ onLoadSearch, currentFilters }: SavedSearchesProps) {
  ...
  const { data: savedSearches = [], isLoading } = trpc.savedSearches.getAll.useQuery();
  ...
  const handleSaveSearch = () => {
    ...
    createSavedSearch.mutate({
      ...
      bedrooms: currentFilters.bedrooms ? Number(currentFilters.bedrooms) : undefined,
      bathrooms: currentFilters.bathrooms ? Number(currentFilters.bathrooms) : undefined,
        ...
        <div className="space-y-4">
          ...
          {!showSaveForm ? (
            ...
            <div className="space-y-3 p-4 bg-card rounded-lg border border-border">
              <div>
                <label className="text-sm font-medium block mb-2">Search Name</label>
                <input
                  type="text"
          ...
          {/* Saved Searches List */}
          {isLoading ? (
            <div className="text-center py-8 text-muted-foreground">Loading saved searches...</div>
          ) : savedSearches.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {showSaveForm ? "Save your first search above" : "No saved searches yet"}
            </div>
          ) : (
            <div className="space-y-2">
              {savedSearches.map((search: any) => (
                <Card key={search.id} className="p-4 flex items-start justify-between hover:shadow-md transition-shadow">
